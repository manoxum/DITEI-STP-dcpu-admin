    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <button 
          ...
          className="px-8 py-4 premium-gradient text-white font-black tracking-widest uppercase text-xs rounded-2xl shadow-xl shadow-emerald-500/20 hover:scale-105 transition-all flex items-center justify-center gap-3"
        >
          {activeTab === 'staff' ? (
      ...
      {(showMemberModal || showInstModal) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 sm:p-24 overflow-y-auto">
          ...
          <motion.div 
            ...
            className="relative bg-white dark:bg-slate-900 rounded-[3rem] w-full max-w-2xl shadow-2xl p-10 md:p-16"
          >
             <button 
               onClick={() => { setShowMemberModal(false); setShowInstModal(false); }}
               className="absolute top-10 right-10 p-3 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-2xl transition-colors"
             >
               <X className="w-6 h-6" />
             </button>
